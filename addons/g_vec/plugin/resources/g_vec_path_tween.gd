@tool
class_name GVecPathTween
extends GVecPathCombination

## Tweens two [GVecPath] baked points.
##
## This resource stores an array of [GVecPath] resources, and generates
...
## The interger part of this parameter selects which [GVecPath] to start
## from, while the fractional part determines how far to the next [GVecPath]
...
func _bake_tweens_from_points(
	...
	if left_points.size() == right_points.size():
		...
		for j in left_points.size():
			...
			start_points.append(left_points[j])
			deltas.append(
					GVecPath.sample_points(right_points, f) - left_points[j])
	else:
		for j in right_points.size():
			var f := float(j) / (right_points.size() - 1)
			var start_point := GVecPath.sample_points(left_points, f)
...
## Combine several segments into one path
