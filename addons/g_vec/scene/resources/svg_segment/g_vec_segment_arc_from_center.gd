...
## Re-parameterize the ellipse, then re-generate the cache of baked points.
## Should be called automatically as needed, no need to call manually.
func bake() -> void:
	_is_dirty = false
	if radii.x != 0 and radii.y != 0:
		parameterize()
	bake_no_parameterize()
...
func parameterize() -> void:
	...
	# Whenever we modify the ellipse parameters without updating the central
	# angle, assume we want to keep the end point as close as possible and
	# re-calculate the central angle based on that.
	if ! _central_angle_is_dirty:
		var new_central_angle_parameter := (
				angle_parameter_to_point(end_point) - _start_angle_parameter)
		while abs(new_central_angle_parameter - central_angle_parameter) > PI:
			new_central_angle_parameter += 2 * PI * signf(
					central_angle_parameter - new_central_angle_parameter)
		central_angle_parameter = new_central_angle_parameter


## Re-generate points without re-parameterizing the ellipse first,
## (but central angle may have changed).
...
func get_vector_out() -> Vector2:
	if _is_dirty:
		bake()
	var end_angle = _start_angle_parameter + central_angle_parameter
...
func _bake_as_line() -> void:
	end_point = lerp(start_point, center, 2.0)
	_baked_points = PackedVector2Array()
