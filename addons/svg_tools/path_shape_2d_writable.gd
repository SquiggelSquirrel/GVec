@tool
class_name PathShape2DWritable
extends PathShape2D

## A [PathShape2D] which simply holds data written to it.
##
## This resource exposes a [member segments] property which represents a set
## of baked points for a [PathShape2D]. It is most commonly used by
## [PathShape2DWithSkeleton], to generate a set of posed points, which can
